...
def find_similar_images(target_image_path, dataset_folder, num_similar=2):
    target_moments = calculate_color_moments(target_image_path)
    images = load_images_from_folder(dataset_folder)

    distances = []
    for filename, img_path in images:
        moments = calculate_color_moments(img_path)
        distance = np.linalg.norm(np.array(target_moments) - np.array(moments))
